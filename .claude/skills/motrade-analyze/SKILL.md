...
价值"的实时定价（GoatBots 自己的"Treasure Chest Booster"页面就有算好的 EV 数字，可以
`WebSearch` "goatbots treasure chest expected value" 查最新值，跟现价对比作为参考，但注意
EV 是"拆包"的期望值，市场价里还包含"不想拆、只想转手"的需求，两者会有偏差，不能直接划等号）。
需求端取决于想拆箱子换取内容物（Play Points/单卡/神器代币等）的玩家数量，供给端主要来自两类
赛事奖励：
- **Challenge**：按最终名次分级发放固定数量（比如报名人数从32涨到50，前16名能拿到的箱子
  数量表是不变的，供给基本是常数），只有"这场赛事因为人数不足被取消没开赛"这种偶发事件才会
  真的减少这部分供给，不是一个逐日变化的连续信号。
- **League 5-0**：每次 5-0 战绩发一次箱子，5-0 的人数逐日真实波动（打的人多、赢得多，5-0
  就多），是目前唯一适合当连续变量追踪的供给端指标——仪表盘 Treasure Chest 详情页会展示这个
  "每日 5-0 数量"曲线（数据来自 `special_items/treasure_chest_supply`，见下面步骤5），写 note
  时可以顺手看一眼这条曲线跟价格走势是否同向变化，作为供给端佐证，但这是辅助参考，不是必须
  每天都要点评的指标。

除了赛事奖励，官方会定期刷新箱内内容概率表（大约每3-4周一次，可以 `WebSearch`
"MTGO Treasure Chest contents update" 查最近一次和下一次刷新日期），这通常是它价格波动最
主要的来源；赛事奖励结构变化（比如 Challenge 改成用箱子发奖还是用 Play Points 发奖）也会
直接影响供给。多数时候它的价格会在一个比较窄的区间里稳定波动，这种"稳定"本身就是正常状态，
不代表有交易信号，不要为了凑研判硬找一个方向性结论。
