...
（CONNECT 403，policy denial）的情况——不是每次都发生，是随机的，跟这次分配到的沙盒有关，
遇到时应该如实报告、不要硬凑数据（历史上已经这样处理过，是对的）。为了不需要用户每次发现
数据没更新都手动找人重跑，cron 现在配置成**每天在同一小时内触发两次**（比如晚上8点和8点半），
第二次本质上是"如果第一次被网络策略挡住，自动重试一次"：
...
3. 如果 `date` 不是今天（不存在，或者是更早的日期——说明昨天/更早的某次也失败过，标记停留
   ...
   第一次触发在 `pipeline.py` 就被网络策略挡死，标记不会被更新，半小时后第二次触发时
   步骤2的判断会正确识别"今天还没跑成功"，重新完整跑一遍（拿到的是另一个沙盒，网络策略
   可能不一样，这就是"自动重试"能生效的原因）。
4. **只要这次运行在 `pipeline.py`（或其他任何关键步骤）遇到网络出口策略拦截而提前退出，
   不管这是当天第一次触发还是第二次触发，都必须立刻用 `PushNotification` 发一条通知**，
   如实说明"今天这次自动更新被网络策略挡住了，数据还是旧的"。**不要自己判断"这是不是第一次
   触发所以先不打扰用户"**——之前出现过这个判断错误：一次运行错误地以为自己是"当天第一次"
   （实际上已经是第二次/最后一次重试），选择了保持沉默，结果两次触发都失败、又都没发通知，
   导致仪表盘数据整整停滞了一天多用户才发现。这个 routine 自己没有可靠的方式区分"这是第几次
   触发"，所以正确做法是**每次被拦截都发**，宁可同一天收到两条重复通知，也不要因为误判
